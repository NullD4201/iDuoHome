...
class GridEditingTest {
    ...
    @Test fun `full page insertion spills past widget cells at start of next page`() {
        ...
        assertEquals((HOME_CELLS - 1).toString(), next.slots[HOME_CELLS + GRID_COLUMNS])
        assertTrue((HOME_CELLS until HOME_CELLS + GRID_COLUMNS).all { next.slots[it] == null })
    }

    @Test fun `resize rejects collisions while movement returns apps to the vacated widget cells`() {
        ...
        val moved = moveWidget(before, 0, 8)
        assertEquals("app", moved.slotAt(0))
        assertNull(moved.slotAt(8))
        assertEquals(2, moved.placement(0)?.row)
    }
    ...
    @Test fun `six row pages gain an empty bottom row without changing page membership`() {
        val old = List<String?>(LEGACY_HOME_CELLS * 2) { "app$it" }
        val migrated = migrateSixRowHomeSlots(old)
        assertEquals("app${LEGACY_HOME_CELLS - 1}", migrated[LEGACY_HOME_CELLS - 1])
        assertTrue((LEGACY_HOME_CELLS until HOME_CELLS).all { migrated[it] == null })
        assertEquals("app$LEGACY_HOME_CELLS", migrated[HOME_CELLS])
        assertEquals(2, homePageCount(migrated.size))
        assertEquals(List(HOME_CELLS - LEGACY_HOME_CELLS) { null },
            migrateSixRowLeadingSlots(List<String?>(LEGACY_HOME_CELLS) { null }).takeLast(HOME_CELLS - LEGACY_HOME_CELLS))
        assertEquals(GRID_ROWS, migrateSixRowOverflowWidget(
            WidgetPlacement(5, 101, 1, 0, LEGACY_GRID_ROWS, GRID_COLUMNS, 4)).row)
    }
}
