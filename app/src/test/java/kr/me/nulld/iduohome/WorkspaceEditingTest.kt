...
class WorkspaceEditingTest {
    @Test fun `widget swap retains bindings and apps`() {
        val before = HomeLayout(listOf(null, null, null, null, null, null, null, null, "a", "b"), listOf("a"), DEFAULT_WIDGET_PLACEMENTS)
        val swapped = moveWidget(before, 0, 2)
        assertEquals(before.placement(0)?.copy(column = 2), swapped.placement(0))
        assertEquals(before.placement(1)?.copy(column = 0), swapped.placement(1))
        assertEquals(before.slots, swapped.slots)
        assertEquals(before.dock, swapped.dock)
    }
    ...
    @Test fun `widget only page permits app drop onto the following new page`() {
        ...
        val nextPage = HOME_CELLS * 2
        assertEquals("a", dropApp(before, "a", DropTarget.Home(nextPage)).slots[nextPage])
    }
