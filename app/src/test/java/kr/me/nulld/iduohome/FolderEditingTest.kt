...
class FolderEditingTest {
    ...
    @Test fun libraryDropExtractsExistingFolderChildWithoutDuplicatingIt() {
        val before = HomeLayout(listOf(folderId), listOf(null, null), folders = listOf(
            FolderEntry(folderId, "Pair", listOf("a", "b"))))
        val next = dropApp(before, "b", DropTarget.Home(9))
        assertEquals("a", next.slotAt(0))
        assertEquals("b", next.slotAt(9))
        assertTrue(next.folders.isEmpty())
        assertEquals(1, next.slots.count { it == "b" })
        val blocked = before.copy(widgetPlacements = listOf(WidgetPlacement(0, 100, 0, 1, 2, 2, 2)))
        assertSame(blocked, dropApp(blocked, "b", DropTarget.Home(9)))
    }
