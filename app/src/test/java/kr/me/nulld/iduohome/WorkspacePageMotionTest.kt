...
class WorkspacePageMotionTest {
    ...
    @Test fun `discover homes and library have the intended endpoints`() {
        ...
        assertEquals(1500f, motion.offset(4f), 0f)
        ...
        assertEquals(4f, motion.position(1500f), 0f)
    }
    ...
    @Test fun `discover is full width and the first library transition is half width`() {
        val single = WorkspacePageMotion(1, 1000f, 500f)
        for (position in listOf(-.5f, 0f, .25f, 1f, 1.75f, 2f, 2.5f)) {
            assertEquals((position - 1f) * if (position < 1f) 1000f else 500f, single.offset(position), 0f)
    ...
    @Test fun `workspace without discover starts at physical page zero`() {
        val noDiscover = WorkspacePageMotion(0, 1000f, 500f)
        ...
        assertEquals(1500f, noDiscover.offset(3f), 0f)
        assertEquals(3f, noDiscover.position(1500f), 0f)
    }
    ...
    @Test fun `half width home panes preserve one to one visual input`() {
        ...
        assertEquals(500f, motion.stride(3, 4), 0f)
    }
}
