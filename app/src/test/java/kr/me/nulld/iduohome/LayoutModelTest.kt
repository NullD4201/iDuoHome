...
class LayoutModelTest {
    ...
    @Test fun `hiding labels preserves row rhythm and large text gains room`() {
        ...
        assertTrue(regular.iconSize / (regular.gridWidth / 4f) in .62f.. .77f)
    }
    ...
    @Test fun `expanded pane appears from actual window width`() {
        ...
        assertEquals(933f / 2f, homeGeometry(933f, 650f, LayoutPreset(), true).homeWidth)
    }
    @Test fun `inner panes use cover column pitch instead of a fixed width cap`() {
        for ((coverWidth, innerWidth) in listOf(416f to 816f, 475f to 933f, 554.67f to 1088f)) {
            val cover = homeGeometry(coverWidth, 840f, LayoutPreset(), true)
            for (innerHeight in listOf(650f, 770f)) for (bottomSpace in listOf(44f, 88f)) {
                val inner = homeGeometry(innerWidth, innerHeight, LayoutPreset(), true,
                    homeBottomSpace = bottomSpace, referenceGridWidth = cover.gridWidth)
                assertEquals(cover.gridWidth / GRID_COLUMNS, inner.gridWidth / GRID_COLUMNS, .01f)
                assertEquals(cover.rowHeight, inner.rowHeight, .01f)
                assertEquals(cover.iconSize, inner.iconSize, .01f)
                assertEquals(cover.widgetHeight, inner.widgetHeight, .01f)
                assertTrue(inner.gridWidth + 68f + 28f <= innerWidth / 2f + .01f)
            }
        }
    }
    @Test fun `a smaller inner window bounds the remembered cover width safely`() {
        val g = homeGeometry(700f, 700f, LayoutPreset(), true, referenceGridWidth = 500f)
        assertEquals(254f, g.gridWidth, .01f)
        assertTrue(g.gridWidth + 68f + 28f <= 350f)
        assertEquals(homeGeometry(1088f, 770f, LayoutPreset(), true),
            homeGeometry(1088f, 770f, LayoutPreset(), true, referenceGridWidth = Float.NaN))
    }
    @Test fun `four through six dock slots fit the cover and inner layouts`() {
        for ((width, height) in listOf(475f to 700f, 933f to 650f)) {
            for (count in MIN_DOCK_SLOTS..MAX_DOCK_SLOTS) {
                val g = homeGeometry(width, height, LayoutPreset(), true, statusHeight = 124f, dockSlots = count)
                assertTrue(g.dockTop >= 124f)
                assertTrue(g.dockTop + g.dockHeight <= height - 124f + .01f)
                assertTrue(g.dockRowHeight >= 48f)
                assertTrue(count * g.dockRowHeight + 16f <= g.dockHeight + .01f)
            }
        }
    }
    ...
    @Test fun `home always has a page independently of the library`() {
        assertEquals(1, homePageCount(0)); assertEquals(1, homePageCount(HOME_CELLS))
        assertEquals(2, homePageCount(HOME_CELLS + 1))
    }
}
