...
data class LauncherState(
    ...
    val foldEffectEnabled: Boolean = false,
    val foldEffectFollowAngle: Boolean = true,
    val foldEffectStrength: Float = .7f,
    ...
    val coverGridWidth get() = (coverWidthDp - compact.dockWidth - 44f)
        .takeIf { coverWidthDp.isFinite() && coverWidthDp in 240f..<650f }
}

private data class WarmLauncherCatalog(
    val entries: List<AppEntry>,
    val profiles: List<AppProfile>,
    val configuration: String,
)

private object LauncherCatalogMemory {
    @Volatile var value: WarmLauncherCatalog? = null
}
...
class LauncherModel(application: Application) : AndroidViewModel(application) {
    ...
    internal val foldEffectSession = FoldEffectSession()
    ...
    private val warmCatalog = LauncherCatalogMemory.value?.takeIf {
        it.configuration == currentIconConfiguration()
    }
    private val mutable = MutableStateFlow(load().let { saved ->
        warmCatalog?.let { saved.copy(apps = it.entries, profiles = it.profiles, loading = false) } ?: saved
    })
    ...
    private val iconCache = mutableMapOf<String, AppEntry>().apply {
        warmCatalog?.entries?.associateByTo(this, AppEntry::id)
    }
    private var iconConfiguration = warmCatalog?.configuration.orEmpty()
    internal val startedWithWarmCatalog = warmCatalog != null
    ...
    fun refresh(invalidatedPackage: String? = null, user: UserHandle = Process.myUserHandle()) {
        ...
        val configuration = currentIconConfiguration()
        viewModelScope.launch {
            try {
                        ...
                        if (activityList == null) emptyList() else activityList.also { authoritativeProfiles += serial }.mapNotNull { info ->
                            ...
                            iconCache[id]?.takeIf { it.label == label && it.available } ?: run {
                                ...
                                val iconBitmap = if (packIcon != null) {
                                    val shaped = createBitmap(144, 144)
                ...
                LauncherCatalogMemory.value = WarmLauncherCatalog(apps.entries, apps.profiles, configuration)
                mutable.update { old ->
                ...
                if (needsMigration && legacyRaw != null && !prefs.contains("state_v1_backup"))
                    prefs.edit {putString("state_v1_backup", legacyRaw)}
    ...
    private fun saveCachedApps(apps: List<AppEntry>) {
        ...
        appCatalogPrefs.edit {putString("apps", array.toString())}
    }
    ...
    private fun currentIconConfiguration(): String {
        val iconPrefs = iconPreferences.state.value
        return getApplication<Application>().resources.configuration.let {
            "${it.densityDpi}|${it.locales.toLanguageTags()}|${it.uiMode}|${iconPrefs.iconPackPackage}|${iconPrefs.iconShape.name}"
        }
    }

    fun setDockCount(value: Int) {
        if (statePayloadInvalid || value !in MIN_DOCK_SLOTS..MAX_DOCK_SLOTS) return
        undoLayout = null
        undoImportSettings = null
        mutable.update { old -> old.copy(dock = resizeDock(old.dock, value), canUndoEdit = false) }
        persist()
    }
    ...
    /** Reveal an existing shortcut, or add it to the first free cell starting on this page. */
    fun addToHome(id: String, preferredPage: Int): Int? {
        val layout = mutable.value.layout
        layout.indexOfShortcut(id)?.let { return it }
        val blocked = layout.widgetPlacements.flatMapTo(mutableSetOf()) { it.coveredIndices() }
        val firstPage = preferredPage.coerceIn(0, layout.pageCount - 1)
        val pages = (firstPage until layout.pageCount) + (0 until firstPage) + layout.pageCount
        val index = pages.asSequence().flatMap { page -> (0 until HOME_CELLS).asSequence().map { homeCellIndex(page, it) } }
            .first { layout.slotAt(it) == null && it !in blocked }
        return index.takeIf { applyDrop(id, DropTarget.Home(it)) }
    }
    ...
    fun createFolder(firstAppId: String, secondAppId: String, targetIndex: Int, title: String = "Folder"): String? {
        ...
        val id = newFolderId()
        return id.takeIf { commitLayout(kr.me.nulld.iduohome.createFolder(mutable.value.layout, FolderEntry(id, title, emptyList()),
            firstAppId, secondAppId, targetIndex)) }
    }
    fun renameFolder(folderId: String, title: String) = commitLayout(kr.me.nulld.iduohome.renameFolder(mutable.value.layout, folderId, title))
    fun addAppToFolder(folderId: String, appId: String, index: Int? = null): Boolean {
        ...
        return commitLayout(kr.me.nulld.iduohome.addAppToFolder(mutable.value.layout, folderId, appId, index))
    }
    fun removeAppFromFolder(folderId: String, appId: String, target: DropTarget) =
        commitLayout(kr.me.nulld.iduohome.removeAppFromFolder(mutable.value.layout, folderId, appId, target))
    fun moveFolderApp(folderId: String, appId: String, index: Int) =
        commitLayout(kr.me.nulld.iduohome.moveFolderApp(mutable.value.layout, folderId, appId, index))
    ...
    fun removePlacements(sources: Set<DropTarget>) = commitLayout(sources.fold(mutable.value.layout, ::removePlacement))
    ...
    fun setDiscoverEnabled(value: Boolean) { if (statePayloadInvalid) return; undoLayout = null; undoImportSettings = null; mutable.update { it.copy(discoverEnabled = value, canUndoEdit = false) }; persist() }
    ...
    fun setFoldEffectEnabled(value: Boolean) { if (statePayloadInvalid) return; mutable.update { it.copy(foldEffectEnabled = value) }; persist() }
    fun setFoldEffectFollowAngle(value: Boolean) { if (statePayloadInvalid) return; mutable.update { it.copy(foldEffectFollowAngle = value) }; persist() }
    fun setFoldEffectStrength(value: Float) {
        if (statePayloadInvalid || !value.isFinite()) return
        mutable.update { it.copy(foldEffectStrength = value.coerceIn(.25f, 1f)) }; persist()
    }
    ...
    fun rememberCoverWidth(widthDp: Float) {
        if (!widthDp.isFinite() || widthDp !in 240f..<650f ||
            kotlin.math.abs(widthDp - mutable.value.coverWidthDp) < .5f) return
        mutable.update { it.copy(coverWidthDp = widthDp) }
        // Device measurement, not a placement edit or portable backup setting.
        prefs.edit { putFloat("cover_width_dp", widthDp) }
    }
    ...
    private fun persist() {
        ...
        val data = JSONObject().put("schema", 9).put("pinned", JSONArray(s.order)).put("homeSlots", JSONArray(s.homeSlots))
            ...
            .put("discoverEnabled", s.discoverEnabled)
            .put("verticalStatus", s.verticalStatus)
            .put("doubleTapToLock", s.doubleTapToLock)
            .put("foldEffectEnabled", s.foldEffectEnabled)
            .put("foldEffectFollowAngle", s.foldEffectFollowAngle)
            .put("foldEffectStrength", s.foldEffectStrength)
    ...
    private fun load(): LauncherState = runCatching {
        ...
        val rawLeadingSlots = if (schema >= 8) {
            val leading = j.optJSONArray("leadingSlots") ?: error("Schema $schema requires a leading slot array")
            val expectedSize = if (schema == 8) LEGACY_HOME_CELLS else HOME_CELLS
            require(leading.length() == expectedSize)
            val loaded = List(expectedSize) { leading.optString(it).takeIf { id -> id.isNotBlank() && id != "null" } }
            if (schema == 8) migrateSixRowLeadingSlots(loaded) else loaded
        } else List(HOME_CELLS) { null }
        val dockArray = j.optJSONArray("dock")
        val dockSize = dockArray?.length()?.coerceIn(MIN_DOCK_SLOTS, MAX_DOCK_SLOTS) ?: MIN_DOCK_SLOTS
        val loadedDock = List(dockSize) { dockArray?.optString(it)?.takeIf { it.isNotBlank() && it != "null" } }
        ...
        val placements = if (schema >= 6) {
            ...
            }.map { if (schema < 9) migrateSixRowOverflowWidget(it) else it }.also { loaded ->
        ...
        val migratedSlots = when (schema) {
            in 2..5 -> migrateSchema5Apps(legacySlots)
            in 6..8 -> migrateSixRowHomeSlots(legacySlots)
            else -> legacySlots
        }
        LauncherState(homeSlots = migratedSlots,
            ...
            discoverEnabled = j.optBoolean("discoverEnabled", true),
            foldEffectEnabled = j.optBoolean("foldEffectEnabled", false),
            foldEffectFollowAngle = j.optBoolean("foldEffectFollowAngle", true),
            foldEffectStrength = j.optDouble("foldEffectStrength", .7).toFloat()
                .let { if (it.isFinite()) it.coerceIn(.25f, 1f) else .7f },
            ...
            coverWidthDp = prefs.getFloat("cover_width_dp", 0f),
...
private fun launcherIcon(drawable: Drawable, shape: IconShape = IconShape.ROUNDED_SQUARE): Bitmap {
    val bitmap = createBitmap(144, 144)
