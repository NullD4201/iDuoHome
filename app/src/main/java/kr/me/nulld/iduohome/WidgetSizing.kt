...
/** Snap the dragged edge to real grid lines, including the taller first two rows. */
internal fun snappedWidgetResize(
    start: WidgetPlacement, horizontalEdge: Int, verticalEdge: Int,
    dx: Float, dy: Float, grid: WidgetGridSizing, limits: WidgetSpanConstraints?,
): WidgetPlacement {
    val minW = limits?.minimum?.width ?: 2
    val minH = limits?.minimum?.height ?: 2
    val maxW = limits?.maximum?.width ?: grid.columns
    val maxH = limits?.maximum?.height ?: grid.rows
    var left = start.column
    var right = start.column + start.spanX
    var top = start.row
    var bottom = start.row + start.spanY
    if (horizontalEdge != 0 && limits?.canResizeHorizontally != false) {
        val change = (dx / grid.cellWidthDp).roundToInt()
        if (horizontalEdge < 0) {
            val range = maxOf(0, right - maxW)..(right - minW)
            if (!range.isEmpty()) left = (left + change).coerceIn(range)
        } else {
            val range = (left + minW)..minOf(grid.columns, left + maxW)
            if (!range.isEmpty()) right = (right + change).coerceIn(range)
        }
    }
    fun rowTop(row: Int) = minOf(row, 2) * grid.topRowHeightDp + maxOf(0, row - 2) * grid.appRowHeightDp
    if (verticalEdge != 0 && limits?.canResizeVertically != false) {
        val line = rowTop(if (verticalEdge < 0) top else bottom) + dy
        val range = if (verticalEdge < 0) maxOf(0, bottom - maxH)..(bottom - minH)
            else (top + minH)..minOf(grid.rows, top + maxH)
        val snapped = range.minByOrNull { kotlin.math.abs(rowTop(it) - line) }
        if (snapped != null) { if (verticalEdge < 0) top = snapped else bottom = snapped }
    }
    return start.copy(column = left, row = top, spanX = right - left, spanY = bottom - top)
}
