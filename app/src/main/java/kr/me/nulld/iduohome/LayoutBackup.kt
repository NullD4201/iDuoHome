...
data class LayoutImportPreview(
    ...
    val profileIssues: List<Pair<String, String>>,
    val appCount: Int,
    val folderCount: Int,
    val widgetCount: Int,
...
fun layoutBackupScope(context: Context): String {
    ...
    return prefs.getString("scope", null) ?: java.util.UUID.randomUUID().toString().also { prefs.edit {putString("scope", it)} }
}
...
fun encodeLayoutBackup(state: LauncherState, widgetDescriptors: List<BackupWidgetDescriptor>, sourceScope: String): String {
    ...
    require(state.dock.size in MIN_DOCK_SLOTS..MAX_DOCK_SLOTS) { "Dock must contain four to six slots" }
...
fun decodeLayoutBackup(raw: String, currentApps: List<AppEntry>, currentProfiles: List<AppProfile>, currentScope: String): LayoutImportPreview {
    ...
    val maximumCells = if (version < 4) LEGACY_HOME_CELLS * 100 else MAX_BACKUP_HOME_CELLS
    require(slotsArray.length() <= maximumCells)
    val storedSlots = List(slotsArray.length()) { index -> if (slotsArray.isNull(index)) null else slotsArray.getString(index) }
    val rawSlots = if (version < 4) migrateSixRowHomeSlots(storedSlots) else storedSlots
    val rawLeadingSlots = if (version == 1) List(HOME_CELLS) { null } else {
        val array = root.getJSONArray("leadingSlots")
        val expectedSize = if (version < 4) LEGACY_HOME_CELLS else HOME_CELLS
        require(array.length() == expectedSize) { "Unfolded-only page must contain exactly $expectedSize cells" }
        val stored = List(expectedSize) { index -> if (array.isNull(index)) null else array.getString(index) }
        if (version < 4) migrateSixRowLeadingSlots(stored) else stored
    }
    ...
    val dockSize = dockArray.length()
    require(if (version < 3) dockSize == MIN_DOCK_SLOTS else dockSize in MIN_DOCK_SLOTS..MAX_DOCK_SLOTS)
    val rawDock = List(dockSize) { index -> if (dockArray.isNull(index)) null else dockArray.getString(index) }
    ...
    val profileIssues = linkedSetOf<Pair<String, String>>()
    ...
    repeat(widgetArray.length()) { index ->
        ...
        val storedPlacement = WidgetPlacement(slot, id, item.strictInt("page"), item.strictInt("column"), item.strictInt("row"),
            item.strictInt("spanX"), item.strictInt("spanY"))
        val placement = if (version < 4) migrateSixRowOverflowWidget(storedPlacement) else storedPlacement
        ...
        val restore = if (id == NEEDS_BINDING_WIDGET) {
            ...
            if ((work && widgetScope != currentScope) || serial !in profileSerials) profileIssues += title to profileLabel
