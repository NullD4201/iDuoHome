    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (intent.action == Intent.ACTION_BATTERY_CHANGED) {
                ...
                val charging = charge == BatteryManager.BATTERY_STATUS_CHARGING ||
                ...
                val samsungHighVoltage = intent.getBooleanExtra("hv_charger", false)
                val samsungChargerType = intent.getIntExtra("charger_type", 0)
                mutable.update { it.copy(battery = if (level >= 0 && scale > 0) (level * 100 / scale).coerceIn(0, 100) else null,
                    charging = charging, fastCharging = charging && isFastCharging(maxCurrent, maxVoltage,
                        samsungHighVoltage, samsungChargerType)) }
            }
            updateConnection()
        }
    }
...
private const val FAST_CHARGING_WATTS_SCALED = 15_000_000_000_000L

internal fun isFastCharging(maxCurrentMicroamps: Int, maxVoltageMicrovolts: Int,
    samsungHighVoltage: Boolean = false, samsungChargerType: Int = 0): Boolean {
    if (samsungHighVoltage || samsungChargerType >= 3) return true
    if (maxCurrentMicroamps <= 0) return false
    val voltage = maxVoltageMicrovolts.takeIf { it > 0 } ?: DEFAULT_CHARGING_VOLTAGE_MICROVOLTS
    return maxCurrentMicroamps.toLong() * voltage >= FAST_CHARGING_WATTS_SCALED
}
