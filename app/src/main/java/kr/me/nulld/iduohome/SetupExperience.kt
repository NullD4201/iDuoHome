...
internal class SetupExperience(context: Context, prefsName: String = PREFS) {
    ...
    fun entryDecision(hadLauncherState: Boolean): SetupEntryDecision {
        ...
        if (decision == SetupEntryDecision.SHOW && !prefs.getBoolean(STARTED, false))
            prefs.edit(commit = true) {putBoolean(STARTED, true)}
        return decision
    }

    fun finish() {
        // Finish before dismissing the sheet so process death cannot make it reappear.
        prefs.edit(commit = true) {putBoolean(FINISHED, true)}
    }
    ...
    Column(
        ...
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.Top) {
            ...
            Column(Modifier.weight(1f)) {
                Text(settingsContext.getString(R.string.settings_welcome), style = MaterialTheme.typography.headlineSmall)
                Text(
                    settingsContext.getString(R.string.settings_welcome_description),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
            IconButton(onClick = onSkip, Modifier.testTag("setup-close")) {
                Icon(Icons.Rounded.Close, settingsContext.getString(R.string.settings_close_welcome))
            }
        }
        ...
        Surface(
            ...
            Column(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                SetupGuideRow(
                    ...
                    title = if (isDefaultHome) settingsContext.getString(R.string.settings_home_is_default) else settingsContext.getString(R.string.settings_choose_iduohome),
                    detail = if (isDefaultHome) settingsContext.getString(R.string.settings_home_button_help)
                        else settingsContext.getString(R.string.settings_home_chooser_help),
                )
                if (!isDefaultHome) Button(
                    onClick = onMakeDefault,
                    modifier = Modifier.fillMaxWidth().heightIn(min = 48.dp).testTag("setup-make-default"),
                ) { Text(settingsContext.getString(R.string.settings_choose_home)) }
                HorizontalDivider()
                SetupGuideRow(
                    icon = Icons.Rounded.Widgets,
                    title = settingsContext.getString(R.string.settings_useful_space),
                    detail = settingsContext.getString(R.string.settings_useful_space_help),
                )
                OutlinedButton(
                    onClick = onAddWidget,
                    modifier = Modifier.fillMaxWidth().heightIn(min = 48.dp).testTag("setup-add-widget"),
                ) { Text(settingsContext.getString(R.string.settings_add_a_widget)) }
            }
        }
        ...
        ) { Text(settingsContext.getString(R.string.settings_explore_home)) }
        TextButton(
            onClick = onSkip,
            modifier = Modifier.align(Alignment.CenterHorizontally).testTag("setup-skip"),
        ) { Text(settingsContext.getString(R.string.settings_not_now)) }
    }
}
