...
class WidgetController(
    ...
    private val bind = activity.registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        ...
        if (result.resultCode == Activity.RESULT_OK && returnedId == pendingId) configure() else {
            if (result.resultCode == Activity.RESULT_OK) failureMessage = activity.getString(R.string.settings_widget_binding_unexpected)
            cancel()
        }
    }
    ...
    fun label(id: Int): String = manager.getAppWidgetInfo(id)?.loadLabel(activity.packageManager) ?: activity.getString(R.string.settings_widget_capital)
    ...
    fun reconfigure(id: Int): Boolean {
        ...
        reconfigureStore.edit {putInt(RECONFIGURE_ID, id)}
    ...
    fun setBuiltin(placement: WidgetPlacement) {
        ...
        if (!model.placeWidget(placement)) failureMessage = activity.getString(NO_ROOM)
    }
    ...
    fun add(placement: WidgetPlacement, provider: AppWidgetProviderInfo, grid: WidgetGridSizing? = null,
        ...
        if (reconfigureWidgetId != null) {
            failureMessage = activity.getString(R.string.settings_finish_open_widget)
            return
        }
        cancel()
        failureMessage = null
    ...
    private fun complete() {
        ...
        if (pendingId < 0 || !originalStillPresent || !model.placeWidget(placement)) {
            failureMessage = activity.getString(if (!originalStillPresent) CHANGED else NO_ROOM)
            cancel()
            return
        }
        clearPending()
    ...
    fun removeSelection(sources: Set<DropTarget>) {
        model.removePlacements(sources)
        reconcileHostIds()
    }
    ...
    private fun clearPending() {
        ...
        setupStatus = null
        pendingOptions = null
        pendingStore.edit {clear()}
        onExternalSetupChanged(false)
    }

    private fun fail() {
        failureMessage = activity.getString(R.string.settings_widget_add_failed)
        cancel()
    }
    ...
    private fun launchReconfigure(): Boolean {
        ...
        if (!canReconfigure(id)) {
            failureMessage = activity.getString(R.string.settings_widget_config_unavailable)
            clearReconfigure()
            return false
        }
        onExternalSetupChanged(true)
        return try {
            ...
            failureMessage = activity.getString(R.string.settings_widget_settings_failed)
            clearReconfigure()
            false
        }
    }
    ...
    private fun persistPending() {
        ...
        pendingStore.edit { clear().putInt(PENDING_ID, pendingId)
            ...
            pendingOptions?.let { options ->
                putInt(PENDING_WIDTH, options.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH))
                    .putInt(PENDING_HEIGHT, options.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_HEIGHT))
            }
            pendingOriginal?.let { putString(PENDING_ORIGINAL, encodePlacement(it)) }
        }
    }
    ...
    companion object {
        ...
        private val NO_ROOM = R.string.settings_widget_no_room
        private val CHANGED = R.string.settings_widget_home_changed
    }
}
