...
internal object LiveDiscover {
    ...
    fun prepare(activity: MainActivity, bounds: Rect, width: Float) {
        if (!activity.discoverEnabled || !attachNativeFeed || externalResultOwners.isNotEmpty() || !DiscoverBounds.available || bounds.isEmpty ||
        ...
        host.get()?.let { current ->
            // Choosing the default Home can create a HOME task alongside the app task.
            // An embedded host in the old task cannot display its feed over the new Home.
            if (current.taskId == activity.taskId && !current.isFinishing && !current.isDestroyed) return
            host.clear()
            current.close()
        }
    ...
    fun page(progress: Float, scrolling: Boolean, towardFeed: Boolean = false) {
        if (owner.get()?.discoverEnabled == false) return
    ...
    fun native(progress: Float): Boolean {
        if (owner.get()?.discoverEnabled == false) return false
    ...
    fun disable(activity: MainActivity) {
        owner = WeakReference(activity)
        startGate.reset()
        ...
        progress = 0f
        nativePosition = 0f
        pagerOwnsMotion = false
        pendingEndpoint = null
        allowNativeOpen = false
        driver.request(0f, false)
    }
...
class LiveDiscoverActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        frame = DiscoverFrame(this, live = true)
        client = DiscoverClient(this,
    ...
    fun close() {
        // Disconnect before a replacement host binds; a delayed detach can close its feed.
        request++
