...
private fun Canvas.copyCurrentMatrixTo(out: Matrix) = getMatrix(out)
...
internal class DiscoverFrame(private val activity: Activity, private val live: Boolean = false) {
    ...
    fun show() {
        ...
        val frame = object : View(activity) {
            ...
            private val traceValues = FloatArray(9)
                ...
                if (live) {
                    ...
                    if (DuoMotionTrace.enabled) {
                        ...
                        if (bucket != lastTraceBucket || liveProgress == 0f || liveProgress == 1f) {
                            ...
                            canvas.copyCurrentMatrixTo(traceMatrix)
                            traceMatrix.getValues(traceValues)
                            ...
                            DuoMotionTrace.event("discover_frame_draw",
                                "live=$liveProgress compose=${LiveDiscover.progress} native=${LiveDiscover.nativePosition} " +
                                    ...
                                    "layer=${layer?.size} matrix=${traceValues.contentToString()}")
                        }
                    }
                    clipPath.rewind()
                    ...
                    canvas.withSave {
                        ...
                        if (layer != null && !layer.isReleased) {
                            ...
                            painter.draw(Density(d), LayoutDirection.Ltr, androidx.compose.ui.graphics.Canvas(canvas),
                                ...
                                drawLauncherBackground(activity, LauncherBackgroundCache.bitmap?.asImageBitmap(), DuoAppearanceRuntime.backgroundDark)
