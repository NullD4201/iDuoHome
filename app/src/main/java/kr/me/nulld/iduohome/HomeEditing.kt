...
internal const val LEGACY_GRID_ROWS = 6
internal const val LEGACY_HOME_CELLS = GRID_COLUMNS * LEGACY_GRID_ROWS
...
fun resizeDock(dock: List<String?>, slots: Int): List<String?> {
    val size = slots.coerceIn(MIN_DOCK_SLOTS, MAX_DOCK_SLOTS)
    return dock.take(size) + List((size - dock.size).coerceAtLeast(0)) { null }
}

/** Adds the new bottom row to every stored six-row page without moving existing shortcuts. */
internal fun migrateSixRowHomeSlots(slots: List<String?>): List<String?> {
    if (slots.isEmpty()) return emptyList()
    val pageCount = (slots.size + LEGACY_HOME_CELLS - 1) / LEGACY_HOME_CELLS
    val migrated = MutableList<String?>(pageCount * HOME_CELLS) { null }
    slots.forEachIndexed { index, id ->
        migrated[index / LEGACY_HOME_CELLS * HOME_CELLS + index % LEGACY_HOME_CELLS] = id
    }
    return normalizeHomeSlots(migrated)
}

internal fun migrateSixRowLeadingSlots(slots: List<String?>): List<String?> {
    require(slots.size == LEGACY_HOME_CELLS)
    return slots + List(HOME_CELLS - LEGACY_HOME_CELLS) { null }
}

internal fun migrateSixRowOverflowWidget(placement: WidgetPlacement): WidgetPlacement =
    if (placement.page > 0 && placement.slot / 3 == placement.page && placement.slot % 3 == 2 &&
        placement.column == 0 && placement.row == LEGACY_GRID_ROWS &&
...
fun canPlaceInDock(layout: HomeLayout, id: String): Boolean =
    id.isNotBlank() && !isReservedFolderId(id) &&
...
fun dropApp(layout: HomeLayout, id: String, target: DropTarget): HomeLayout {
    if (id.isBlank()) return layout
    layout.folders.firstOrNull { id in it.appIds }?.let { folder ->
        return removeAppFromFolder(layout, folder.id, id, target)
    }
    return when (target) {
        is DropTarget.Home -> {
...
fun moveWidget(layout: HomeLayout, slot: Int, index: Int): HomeLayout {
    ...
    val moved = old.copy(page = page, column = local % GRID_COLUMNS,
        row = local / GRID_COLUMNS, spanY = old.spanY.coerceAtMost(GRID_ROWS))
    if (!moved.valid() || moved == old) return layout
    val collisions = layout.widgetPlacements.filter { it.slot != slot && overlaps(it, moved) }
    if (collisions.size > 1) return layout
    val other = collisions.singleOrNull()
    val swapped = other?.copy(page = old.page, column = old.column, row = old.row)
    if (swapped != null && !swapped.valid()) return layout
    val placements = layout.widgetPlacements.map {
        when (it.slot) { slot -> moved; other?.slot -> swapped!!; else -> it }
    }
    if (placements.any { a -> placements.any { b -> a.slot != b.slot && overlaps(a, b) } }) return layout
    val occupied = placements.flatMapTo(mutableSetOf()) { it.coveredIndices() }
    val displaced = (moved.coveredIndices() + (swapped?.coveredIndices() ?: emptySet()))
        .sorted().mapNotNull { cell -> layout.slotAt(cell)?.let { cell to it } }
    // Only cells freed by this move are used; unrelated icons keep their positions.
    val vacancies = (old.coveredIndices() + (other?.coveredIndices() ?: emptySet())).sorted()
        .filter { it !in occupied && layout.slotAt(it) == null }
    if (vacancies.size < displaced.size) return layout
    var result = layout.copy(widgetPlacements = placements)
    displaced.forEach { (cell, _) -> result = result.withSlot(cell, null) }
    displaced.zip(vacancies).forEach { (app, cell) -> result = result.withSlot(cell, app.second) }
    return result
}
