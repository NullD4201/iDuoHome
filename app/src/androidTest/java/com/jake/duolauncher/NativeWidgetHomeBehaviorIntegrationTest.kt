...
class NativeWidgetHomeBehaviorIntegrationTest {
    ...
    @Test fun homeDuringRequiredConfigurationReturnsToLiveLauncherAndResolvesOneId() {
        ...
        val hadBindGrant = shell("dumpsys appwidget").lineSequence().any {
            it.contains("user=0 package=com.jake.duolauncher")
        }
        ...
        try {
            ...
            shell("appwidget grantbind --package com.jake.duolauncher --user 0")
            ...
            val provider = widgets.personalProviders().single {
                it.provider.packageName == "com.jake.duolauncher.test" &&
            ...
            await { automation.rootInActiveWindow?.packageName == "com.jake.duolauncher.test" }
            ...
            val reachedHome = runCatching {
                await(8_000) { automation.rootInActiveWindow?.packageName == "com.jake.duolauncher" }; true
            }.getOrDefault(false)
            ...
            assertTrue("MainActivity was not resumed after HOME; see native-widget-home-$variant.txt",
                afterHome.lineSequence().any { line ->
                    (line.contains("mResumedActivity") || line.contains("topResumedActivity")) &&
                        line.contains("com.jake.duolauncher/.MainActivity")
                })
            ...
            cleanup { shell("appwidget ${if (hadBindGrant) "grantbind" else "revokebind"} --package com.jake.duolauncher --user 0") }
            cleanup {
