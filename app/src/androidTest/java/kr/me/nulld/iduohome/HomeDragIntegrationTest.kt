...
class HomeDragIntegrationTest {
    ...
    @Test fun draggedAppTracksTheFingerWithoutAWindowInsetOffset() {
        ready()
        val before = model().state.value.layout
        try {
            val app = model().state.value.apps.first()
            compose.runOnIdle { model().setPinned(app.id, true) }
            compose.waitForIdle()
            val source = model().state.value.homeSlots.indexOf(app.id)
            val rootBounds = root().fetchSemanticsNode().boundsInRoot
            val start = center("home-cell-$source") - rootBounds.topLeft
            val pointer = start + Offset(80f, 120f)
            root().performTouchInput {
                down(start); advanceEventTime(700); moveTo(pointer, 100)
            }
            compose.waitForIdle()
            val ghost = compose.onNodeWithTag("drag-ghost").fetchSemanticsNode().boundsInRoot
            val fingerY = pointer.y + rootBounds.top
            assertEquals("fingerY=$fingerY ghost=$ghost", fingerY - ghost.height * .15f, ghost.center.y, 2f)
    ...
    @Test fun heldDragTurnsPageAndPersistsSparseNewPage() {
        ...
        try {
            ...
            compose.waitForIdle()
            val destinationIndex = HOME_CELLS * 2 - 1
            val destination = center("home-cell-$destinationIndex")
            ...
            assertEquals(id, model().state.value.homeSlots[destinationIndex])
            val saved = org.json.JSONObject(compose.activity.getSharedPreferences("launcher", 0).getString("state", "{}")!!)
            assertEquals(id, saved.getJSONArray("homeSlots").getString(destinationIndex))
