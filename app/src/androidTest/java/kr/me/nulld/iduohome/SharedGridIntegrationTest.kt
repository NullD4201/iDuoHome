...
class SharedGridIntegrationTest {
    ...
    @Test fun realWidgetResizesReportsSizeSurvivesRecreationAndRemoveUndoKeepsBinding() {
        ...
        try {
            ...
            compose.onNodeWithTag("widget-options-popup-$slot").assertIsDisplayed()
            pressBack()
            compose.onNodeWithTag("widget-options-popup-$slot").assertDoesNotExist()
            compose.onNodeWithTag("widget-slot-$slot").performTouchInput { longClick() }
            compose.onNodeWithTag("widget-options-popup-$slot").assertIsDisplayed()
    ...
    @Test fun nativeWidgetUsesTheExactEmptyAnchorInsteadOfSearchingElsewhere() {
        ...
        try {
            ...
            compose.runOnIdle { model().applyDrop(pageAnchor, DropTarget.Home(HOME_CELLS * 2 - 1)) }
            compose.onNodeWithContentDescription("Home page 2").performClick()
            compose.waitForIdle()
    ...
    @Test fun schema5MigratesBacksUpAndRoundTripsWithExactCoordinates() {
        ...
        try {
            prefs.edit().putString("state", legacy).remove("state_v5_backup").remove("state_v6_backup")
                .remove("state_v7_backup").remove("state_v8_backup")
            ...
            compose.waitUntil(15_000) { !migrated.state.value.loading && JSONObject(prefs.getString("state", "{}")!!).optInt("schema") == 9 }
            ...
            assertEquals(legacy, prefs.getString("state_v8_backup", null))
            ...
            compose.runOnIdle { roundTripped = ViewModelProvider(roundTripStore, factory)["schema9-roundtrip", LauncherModel::class.java] }
    ...
    @Test fun schema7MigrationCreatesEmptyLeadingGridRemovesOnlyExactDemoAndRetainsRealLeadingWidget() {
        ...
        try {
            prefs.edit().putString("state", legacy).remove("state_v7_backup").remove("state_v8_backup")
                .putBoolean("initialized", true).commit()
            ...
            compose.waitUntil(15_000) { !migrated.state.value.loading &&
                JSONObject(prefs.getString("state", "{}")!!).optInt("schema") == 9 }
            assertEquals(List(HOME_CELLS) { null }, migrated.state.value.leadingSlots)
            ...
            compose.runOnIdle { reloaded = ViewModelProvider(reloadStore!!, factory)["schema9-leading-reload", LauncherModel::class.java] }
    ...
    @Test fun malformedSavedRecordsFailClosedWhileSchema6UpgradesAndSchema7FoldersRoundTrip() {
        ...
        val invalid = listOf(
            ...
            base().put("widgets", JSONArray().put(widget(spanY = GRID_ROWS + 1))),
            base().put("widgets", JSONArray().put(widget(column = 3, spanX = 2))),
            base().put("widgets", JSONArray().put(widget(row = GRID_ROWS - 1, spanY = 2))),
        ...
        try {
            ...
            val positive = listOf(
                ...
                base().put("widgets", JSONArray().put(widget(slot = 5, id = 9, page = 1, row = LEGACY_GRID_ROWS, spanX = 4, spanY = 4))) to
                    listOf(WidgetPlacement(5, 9, 1, 0, GRID_ROWS, 4, 4)),
            )
