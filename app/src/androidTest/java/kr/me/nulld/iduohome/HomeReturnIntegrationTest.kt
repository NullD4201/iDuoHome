...
class HomeReturnIntegrationTest {
    ...
    private fun await(timeout: Long = 15_000, condition: () -> Boolean) {
        ...
        while (!condition()) {
            check(SystemClock.uptimeMillis() < deadline) { "Home return timed out: message=${LiveDiscover.message.value}, page=${LiveDiscover.progress}, native=${LiveDiscover.nativePosition}" }
    ...
    @Test fun choosingDefaultHomeMovesDiscoverFromTheAppTaskToTheHomeTask() {
        ...
        val otherHome = previousHome.takeIf { it.isNotEmpty() && it != context.packageName }
            ?: "com.google.android.apps.nexuslauncher"
        val attachBefore = LiveDiscover.attachNativeFeed
        LiveDiscover.attachNativeFeed = true
        var appMain: MainActivity? = null
        var appHost: LiveDiscoverActivity? = null
        var discoverBefore: Boolean? = null
        try {
            shell("cmd role add-role-holder android.app.role.HOME $otherHome 0")
            context.startActivity(Intent.makeMainActivity(android.content.ComponentName(context, MainActivity::class.java))
                .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
            await { LiveDiscover.owner.get()?.let { !it.isFinishing && !it.isDestroyed &&
                it.intent.hasCategory(Intent.CATEGORY_LAUNCHER) } == true }
            appMain = checkNotNull(LiveDiscover.owner.get())
            instrumentation.runOnMainSync {
                val model = ViewModelProvider(appMain)[LauncherModel::class.java]
                discoverBefore = model.state.value.discoverEnabled
                model.setDiscoverEnabled(true)
            }
            await { LiveDiscover.host.get()?.taskId == appMain.taskId }
            appHost = checkNotNull(LiveDiscover.host.get())
            click("Discover")
            await(20_000) { LiveDiscover.message.value == null && LiveDiscover.nativePosition >= .99f }
            click("Home page 1")
            await { progressAtHome() }
            val before = context.getSharedPreferences("launcher", 0).getString("state", null)

            // A real HOME launch creates a different task even though Main uses singleTask.
            shell("cmd role add-role-holder android.app.role.HOME ${context.packageName} 0")
            shell("input keyevent KEYCODE_HOME")
            await { LiveDiscover.owner.get()?.let { it.taskId != appMain.taskId &&
                it.intent.hasCategory(Intent.CATEGORY_HOME) } == true }
            val homeMain = checkNotNull(LiveDiscover.owner.get())
            val homeModel = ViewModelProvider(homeMain)[LauncherModel::class.java]
            org.junit.Assert.assertTrue("A new HOME task must reuse the in-process app catalog",
                homeModel.startedWithWarmCatalog)
            org.junit.Assert.assertFalse("A new HOME task must not show the initial loader",
                homeModel.state.value.loading)
            await { LiveDiscover.host.get()?.taskId == homeMain.taskId }
            assertNotSame(appHost, LiveDiscover.host.get())
            repeat(2) {
                click("Discover")
                await(20_000) { LiveDiscover.message.value == null && LiveDiscover.nativePosition >= .99f }
                shell("input keyevent KEYCODE_HOME")
                await { progressAtHome() && LiveDiscover.host.get()?.taskId == homeMain.taskId }
            }
            org.junit.Assert.assertEquals(before, context.getSharedPreferences("launcher", 0).getString("state", null))
        } finally {
            LiveDiscover.attachNativeFeed = false
            discoverBefore?.let { enabled -> instrumentation.runOnMainSync {
                LiveDiscover.owner.get()?.let { ViewModelProvider(it)[LauncherModel::class.java].setDiscoverEnabled(enabled) }
            } }
            shell("cmd role add-role-holder android.app.role.HOME $otherHome 0")
            try {
                instrumentation.runOnMainSync {
                    appMain?.finish(); appHost?.finish()
                    LiveDiscover.owner.get()?.finish(); LiveDiscover.host.get()?.finish()
                }
            } finally {
                if (previousHome.isNotEmpty()) shell("cmd role add-role-holder android.app.role.HOME $previousHome 0")
                else shell("cmd role remove-role-holder android.app.role.HOME $otherHome 0")
                LiveDiscover.attachNativeFeed = attachBefore
            }
        }
    }

    private fun progressAtHome() = LiveDiscover.progress == 0f && LiveDiscover.nativePosition == 0f && !LiveDiscover.pagerOwnsMotion
    ...
    @Test fun systemHomeKeepsTheSelectedHomePageAndReturnsOtherSurfacesToIt() {
        ...
        val previousHome = shell("cmd role get-role-holders android.app.role.HOME")
            .lineSequence().firstOrNull().orEmpty()
        shell("cmd role add-role-holder android.app.role.HOME kr.me.nulld.iduohome 0")
        ...
        try {
            ...
            val completedRefreshes = ViewModelProvider(selectedOwner)[LauncherModel::class.java].completedRefreshes
            ...
            org.junit.Assert.assertEquals(completedRefreshes,
                ViewModelProvider(selectedOwner)[LauncherModel::class.java].completedRefreshes)
            ...
            val cleanupHome = previousHome.takeIf { it.isNotEmpty() && it != "kr.me.nulld.iduohome" }
                ?: "com.google.android.apps.nexuslauncher"
