...
class NativeWidgetHomeBehaviorIntegrationTest {
    ...
    @Test fun homeDuringRequiredConfigurationReturnsToLiveLauncherAndResolvesOneId() {
        ...
        val hadBindGrant = shell("dumpsys appwidget").lineSequence().any {
            it.contains("user=0 package=kr.me.nulld.iduohome")
        }
        ...
        try {
            ...
            shell("appwidget grantbind --package kr.me.nulld.iduohome --user 0")
            ...
            val provider = widgets.personalProviders().single {
                it.provider.packageName == "kr.me.nulld.iduohome.test" &&
            ...
            instrumentation.runOnMainSync {
                ...
                assertTrue(model.applyDrop(app.id, DropTarget.Home(HOME_CELLS * 2 - 1)))
            ...
            await { automation.rootInActiveWindow?.packageName == "kr.me.nulld.iduohome.test" }
            ...
            val reachedHome = runCatching {
                await(8_000) { automation.rootInActiveWindow?.packageName == "kr.me.nulld.iduohome" }; true
            }.getOrDefault(false)
            ...
            assertTrue("HOME did not visibly return to iDuoHome; see native-widget-home-$variant.txt", reachedHome)
            assertTrue("MainActivity was not resumed after HOME; see native-widget-home-$variant.txt",
                afterHome.lineSequence().any { line ->
                    (line.contains("mResumedActivity") || line.contains("topResumedActivity")) &&
                        line.contains("kr.me.nulld.iduohome/.MainActivity")
                })
            ...
            cleanup { shell("appwidget ${if (hadBindGrant) "grantbind" else "revokebind"} --package kr.me.nulld.iduohome --user 0") }
            cleanup {
