...
class LeadingLayoutBackupIntegrationTest {
    ...
    @Test fun version4RoundTripsMixedLeadingAppsFoldersWidgetsAndProfiles() {
    ...
    @Test fun version1ImportsWithCanonicalEmptyLeadingPageAndVersion2RequiresIt() {
        ...
        val version3 = JSONObject(encodeLayoutBackup(state, emptyList(), "scope"))
            .put("version", 3)
            .put("leadingSlots", JSONArray(state.leadingSlots.take(LEGACY_HOME_CELLS)))
        val migrated = decodeLayoutBackup(version3.toString(), state.apps, profiles, "scope")
        assertEquals(state.leadingSlots, migrated.layout.leadingSlots)
    }
