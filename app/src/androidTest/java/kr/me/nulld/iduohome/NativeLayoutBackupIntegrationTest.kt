...
class NativeLayoutBackupIntegrationTest {
    ...
    private fun clearFixtureDocument() {
        ...
        shell("run-as kr.me.nulld.iduohome.test mkdir -p files")
        shell("run-as kr.me.nulld.iduohome.test touch files/$marker")
        shell("run-as kr.me.nulld.iduohome.test rm -f files/saf-layout.json")
        val files = shell("run-as kr.me.nulld.iduohome.test ls files").lineSequence().toSet()
        try {
            ...
            shell("run-as kr.me.nulld.iduohome.test rm -f files/$marker")
        }
    }
    ...
    private fun openFixtureRoot() {
        ...
        clickLastActive("iDuoHome Backup Fixture")
    }
