...
class ExpandedWorkspaceIntegrationTest {
    ...
    private fun assertDisplayedCount(tag: String, expected: Int) {
        ...
        val visible = nodes.fetchSemanticsNodes(atLeastOneRootRequired = false)
            .mapIndexed { index, node ->
                val bounds = node.boundsInRoot
                val full = nodes[index].getUnclippedBoundsInRoot()
                val fullWidth = (full.right - full.left).value * compose.density.density
                // Cached neighbors can expose a narrow padding sliver at the viewport edge.
                bounds.width >= fullWidth / 2f && bounds.height > 1f && bounds.overlaps(viewport)
            }.count { it }
    ...
    @Test fun expandedPagerShowsUniqueOverlappingPairsAndLongSwipesMoveOnePage() {
        ...
        try {
            ...
            assertDisplayedCount("home-cell-$HOME_CELLS", 1)
            assertDisplayedCount("home-cell-${HOME_CELLS * 2}", 1)
    ...
    @Test fun coverRoundTripKeepsTheRightPagePreferencesDockAndSplitLibrary() {
        ready(); val before = model().state.value.layout
        ...
        try {
            ...
            assertEquals(saved, preferences())
            val returnedDock = compose.onNodeWithTag("dock").fetchSemanticsNode().boundsInRoot
            assertEquals(innerDock.left, returnedDock.left, 2f)
            assertEquals(innerDock.right, returnedDock.right, 2f)
            ...
            assertTrue("All apps occupies one pane", library.width in (viewport.width * .35f)..(viewport.width * .55f))
            val home = compose.onNodeWithTag("home-page-2").assertIsDisplayed().fetchSemanticsNode().boundsInRoot
            assertTrue("The last Home page remains beside All apps", home.right <= library.left)
            assertEquals(saved, preferences())
            shell("wm size 1248x1972")
            compose.waitUntil(15000) { compose.activity.windowManager.currentWindowMetrics.bounds.width() == 1248 }
            compose.waitForIdle()
            assertEquals("All apps", page())
            val coverLibrary = compose.onNodeWithTag("library-page").fetchSemanticsNode().boundsInRoot
            val coverViewport = pager().fetchSemanticsNode().boundsInRoot
            assertTrue("Cover display retains the full-width list", coverLibrary.width > coverViewport.width * .9f)
            assertEquals(saved, preferences())
        } finally {
