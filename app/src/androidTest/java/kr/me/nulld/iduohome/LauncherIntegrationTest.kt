...
class LauncherIntegrationTest {
    ...
    @Test fun systemStatusBarStaysVisibleWithEitherRailSettingAndAfterRecreation() {
        ready()
        ...
        val railBefore = model().state.value.verticalStatus
        fun assertStatusBarVisible() {
            compose.waitUntil(5_000) {
                ViewCompat.getRootWindowInsets(compose.activity.window.decorView)
                    ?.isVisible(WindowInsetsCompat.Type.statusBars()) == true
            }
            val topInset = ViewCompat.getRootWindowInsets(compose.activity.window.decorView)!!
                .getInsets(WindowInsetsCompat.Type.statusBars()).top
            org.junit.Assert.assertTrue("Status bar must reserve space", topInset > 0)
            org.junit.Assert.assertTrue("Home must stay below the status bar",
                compose.onNodeWithTag("app-pager").fetchSemanticsNode().boundsInRoot.top >= topInset)
        }
        try {
            for (railEnabled in listOf(true, false)) {
                compose.runOnIdle { model().setVerticalStatus(railEnabled) }
            ...
            compose.runOnIdle { model().setVerticalStatus(railBefore) }
        }
    }
