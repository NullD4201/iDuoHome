...
class AppearanceBackgroundIntegrationTest {
    ...
    @Test fun defaultAndSystemBackgroundChoicesPersistAcrossRecreation() {
        ready()
        val prefs = launcherBackgroundPreferences(compose.activity)
        val oldPrefs = prefs.all.toMap()
        val file = launcherBackgroundFile(compose.activity)
        val oldPhoto = file.takeIf(File::isFile)?.readBytes()
        try {
            prefs.edit().putBoolean("photoEnabled", false).remove("useSystemWallpaper").commit()
            compose.runOnUiThread { LauncherBackgroundCache.changed(null) }
            compose.activityRule.scenario.recreate(); ready(); settings()
            compose.onNodeWithTag("background-reset").assertIsNotEnabled()
            compose.onNodeWithTag("background-system").performScrollTo().performClick()
            assertTrue(launcherSystemBackgroundEnabled(compose.activity))
            compose.activityRule.scenario.recreate(); ready(); settings()
            compose.onNodeWithTag("background-system").assertIsNotEnabled()
            compose.onNodeWithTag("background-reset").performScrollTo().performClick()
            assertFalse(launcherSystemBackgroundEnabled(compose.activity))
            compose.activityRule.scenario.recreate(); ready(); settings()
            compose.onNodeWithTag("background-reset").assertIsNotEnabled()
            compose.onNodeWithTag("background-system").assertIsEnabled()
        } finally {
    ...
    @Test fun realPhotoPickerSelectsTheExactOwnedMediaItem() {
        ...
        val values = ContentValues().apply {
            ...
            put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/iDuoHomeTests")
